...
class WSEDMS {
    ...
    getEDMSDocuments(objectID, objectType, mode, config = {}) {
        ...
        return ajax.get(process.env.REACT_APP_BACKEND.replace('/eamlightws/rest', '/cern-eam-services/rest') + `/edms/documents?objectid=${objectID}&objectType=${objectType}&mode=${mode}`, config);
    }

    createNewDocument(data, config = {}) {
        return ajax.post(process.env.REACT_APP_BACKEND.replace('/eamlightws/rest', '/cern-eam-services/rest') + `/edms/createdocument`, data, config);
    }
