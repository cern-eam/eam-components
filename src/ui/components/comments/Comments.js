...
class Comments extends Component {
    ...
    componentDidUpdate(prevProps) {
        //Just read for existing object
        if (
            (prevProps.entityKeyCode !== this.props.entityKeyCode || prevProps.entityCode !== this.props.entityCode) &&
    ...
    updateComment = (comment) => {
        const canUpdate =
            !this.props.restrictEditToCreator ||
            (this.props.currentUsrCode && this.props.currentUsrCode === comment.creationUserCode);
        if (canUpdate) {
            //Remove pk property
            delete comment.pk;
            ...
            this.props
                .updateComment(comment)
                .then((response) => {
                    this.setState(() => ({ comments: response.body.data }));
            ...
            <List style={{ width: '100%' }}>
                {this.state.comments.map((comment) => (
                    <Comment
                        ...
                        restrictEditToCreator={this.props.restrictEditToCreator}
                        currentUsrCode={this.props.currentUsrCode}
                    />
                ))}
