...
var EAMFilterField = function EAMFilterField(_ref6) {
  ...
  var handleMultiFilterCheckboxChange = React.useCallback(function (event, value, label) {
    if (event.target.checked) {
      setMultiSelectFilter(function (prev) {
        var multiSelectValues = prev.fieldValue ?? [];
        updateMultiSelectFilter([].concat(_toConsumableArray(multiSelectValues), [value]));
        return _objectSpread({}, prev, {
          fieldValue: multiSelectValues,
          joiner: "OR"
        });
      });
      ...
      setMultiSelectFilter(function (prev) {
        var multiSelectValues = prev.fieldValue.filter(function (item) {
          return item !== value;
        });
